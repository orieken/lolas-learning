...
const html = fs.readFileSync(path.resolve(__dirname, '../apps/games/word-search/word_search_puzzles.html'),'utf8');
...
for (let i=0;i<sections.length;i++){
  ...
  const cols=12; const rows=Math.ceil(cells.length/cols);
  const matrix=[]; for(let r=0;r<rows;r++){ const row=[]; for(let c=0;c<cols;c++){ const idx=r*cols+c; row.push(idx<cells.length?cells[idx]:''); } matrix.push(row); }
